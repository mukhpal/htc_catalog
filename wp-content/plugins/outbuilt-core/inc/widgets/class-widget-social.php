...
if (!class_exists('Outbuilt_Addon_Social_Widget')) {
	class Outbuilt_Addon_Social_Widget extends WP_Widget {
		...
		public function __construct() {
			...
			$this->social_services_array = apply_filters('outbuilt_social_widget_profiles', array(
				'twitter' => array(
					'name' => 'Twitter',
					'url'  => '',
				),
			...
			parent::__construct(
				'outbuilt_social',
				esc_html__('&rarr; Social Icons', 'outbuilt-core'),
				array(
					'classname'   => 'widget-outbuilt-social social-widget',
					'description' => esc_html__('Display your social media icons.', 'outbuilt-core'),
		...
		public function social_widget_style() { ?>
			<style>
				.outbuilt-social-widget-services-list {
					padding-top: 10px;
				}

				.outbuilt-social-widget-services-list li {
					cursor: move;
					background: #fafafa;
					padding: 10px;
				...
				.outbuilt-social-widget-services-list li p {
					margin: 0
				}

				.outbuilt-social-widget-services-list li label {
					margin-bottom: 3px;
					display: block;
					color: #222;
				}

				.outbuilt-social-widget-services-list li label span.fa {
					margin-right: 10px
				}

				.outbuilt-social-widget-services-list .placeholder {
					border: 1px dashed #e3e3e3
				}

				.outbuilt-widget-select {
					width: 100%
				}
			...
			<script>
				(function($) {
					$(document).ajaxSuccess(function(e, xhr, settings) {
						var widget_id_base = 'outbuilt_social';
						if (settings.data.search('action=save-widget') != -1 && settings.data.search('id_base=' + widget_id_base) != -1) {
							tjSortServices();
						}
					});

					function tjSortServices() {
						$('.outbuilt-social-widget-services-list').each(function() {
							var id = $(this).attr('id');
							$('#' + id).sortable({
		...
		?>
			<ul class="outbuilt-social-icons <?php echo esc_attr($transition); ?> style-<?php echo esc_attr($style); ?>">
				<?php
				// Original Array
				...
				foreach ($social_services as $key => $val) {
					...
					if ($link) {
						...
						echo '<li class="outbuilt-' . esc_attr($key) . '">';
		...
		public function form($instance) {
			...
			$instance = wp_parse_args((array) $instance, array(
				'title'           	 => esc_attr__('Follow Us', 'outbuilt-core'),
				'style' 	  		 => esc_html__('Light', 'outbuilt-core'),
				'transition' 	  	 => esc_html__('Rotate', 'outbuilt-core'),
				'font_size'       	 => '',
				'border_radius'   	 => '',
				'bg_color'   	     => '',
				'bg_hover_color'   	 => '',
			...
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style:', 'outbuilt-core'); ?></label>
				<select class='widefat' name="<?php echo esc_attr($this->get_field_name('style')); ?>" id="<?php echo esc_attr($this->get_field_id('style')); ?>">
					<option value="light" <?php selected($instance['style'], 'light') ?>><?php esc_html_e('Light', 'outbuilt-core'); ?></option>
					<option value="dark" <?php selected($instance['style'], 'dark') ?>><?php esc_html_e('Dark', 'outbuilt-core'); ?></option>
					<option value="colored" <?php selected($instance['style'], 'colored') ?>><?php esc_html_e('Colored', 'outbuilt-core'); ?></option>
					<option value="simple" <?php selected($instance['style'], 'simple') ?>><?php esc_html_e('Simple', 'outbuilt-core'); ?></option>
				</select>
			</p>

			<p>
				<label for="<?php echo esc_attr($this->get_field_id('transition')); ?>"><?php esc_html_e('Transition:', 'outbuilt-core'); ?></label>
				<select class='widefat' name="<?php echo esc_attr($this->get_field_name('transition')); ?>" id="<?php echo esc_attr($this->get_field_id('transition')); ?>">
					<option value="no-transition" <?php selected($instance['transition'], 'no-transition') ?>><?php esc_html_e('None', 'outbuilt-core'); ?></option>
					<option value="float" <?php selected($instance['transition'], 'float') ?>><?php esc_html_e('Float', 'outbuilt-core'); ?></option>
					<option value="rotate" <?php selected($instance['transition'], 'rotate') ?>><?php esc_html_e('Rotate', 'outbuilt-core'); ?></option>
					<option value="zoomout" <?php selected($instance['transition'], 'zoomout') ?>><?php esc_html_e('Zoom Out', 'outbuilt-core'); ?></option>
				</select>
			</p>

			<p>
				<label for="<?php echo esc_attr($this->get_field_id('target')); ?>"><?php esc_html_e('Link Target', 'outbuilt-core'); ?>:</label>
				<br />
				<select class="outbuilt-widget-select" name="<?php echo esc_attr($this->get_field_name('target')); ?>" id="<?php echo esc_attr($this->get_field_id('target')); ?>">
					<option value="blank" <?php selected($instance['target'], 'blank') ?>><?php esc_html_e('Blank', 'outbuilt-core'); ?></option>
					<option value="self" <?php selected($instance['target'], 'self') ?>><?php esc_html_e('Self', 'outbuilt-core'); ?></option>
				</select>
			</p>

			<p>
				<label for="<?php echo esc_attr($this->get_field_id('size')); ?>"><?php esc_html_e('Dimensions', 'outbuilt-core'); ?>:</label>
				...
				<small><?php esc_html_e('Example:', 'outbuilt-core'); ?> 40px</small>
			</p>

			<p>
				<label for="<?php echo esc_attr($this->get_field_id('font_size')); ?>"><?php esc_html_e('Font Size', 'outbuilt-core'); ?>:</label>
				...
				<small><?php esc_html_e('Example:', 'outbuilt-core'); ?> 18px</small>
			</p>

			<p>
				<label for="<?php echo esc_attr($this->get_field_id('border_radius')); ?>"><?php esc_html_e('Border Radius', 'outbuilt-core'); ?></label>
				...
				<small><?php esc_html_e('Example:', 'outbuilt-core'); ?> 4px</small>
			</p>

			<p>
				<label class="color-label" for="<?php echo esc_attr($this->get_field_id('bg_color')); ?>"><?php esc_html_e('Background Color', 'outbuilt-core'); ?></label>
			...
			<p>
				<label class="color-label" for="<?php echo esc_attr($this->get_field_id('bg_hover_color')); ?>"><?php esc_html_e('Background Color Hover', 'outbuilt-core'); ?></label>
			...
			<p>
				<label class="color-label" for="<?php echo esc_attr($this->get_field_id('color')); ?>"><?php esc_html_e('Color', 'outbuilt-core'); ?></label>
			...
			<p>
				<label class="color-label" for="<?php echo esc_attr($this->get_field_id('color_hover')); ?>"><?php esc_html_e('Color Hover', 'outbuilt-core'); ?></label>
			...
			<p>
				<label class="color-label" for="<?php echo esc_attr($this->get_field_id('border_color')); ?>"><?php esc_html_e('Border Color', 'outbuilt-core'); ?></label>
			...
			<p>
				<label class="color-label" for="<?php echo esc_attr($this->get_field_id('border_hover_color')); ?>"><?php esc_html_e('Border Color Hover', 'outbuilt-core'); ?></label>
			...
			<h3 style="margin-top:20px;margin-bottom:0;"><?php esc_html_e('Social Links', 'outbuilt'); ?></h3>
			<ul id="<?php echo esc_attr($field_id_services); ?>" class="outbuilt-social-widget-services-list">
				<input type="hidden" id="<?php echo esc_attr($field_name_services); ?>" value="<?php echo esc_attr($field_name_services); ?>">
				<input type="hidden" id="<?php echo esc_attr(wp_create_nonce('outbuilt_fontawesome_social_widget_nonce')); ?>">
				<?php
				// Social array
				$social_services_array = $this->social_services_array;
		...
		public function colors($args, $instance) {
			...
			if (
				...
				<style>
					#<?php echo $id; ?>.widget-outbuilt-social ul li a {
						<?php if ($bg_color) {
					...
					#<?php echo $id; ?>.widget-outbuilt-social ul li a:hover {
...
register_widget('Outbuilt_Addon_Social_Widget');
